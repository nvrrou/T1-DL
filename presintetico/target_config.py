"""Objetivo comun: tres clases de cambio porcentual del precio a 7 dias."""

import numpy as np
import pandas as pd

CLASSIFICATION_TARGET = "target_price_direction_7d"
NEUTRAL_CHANGE_PCT = 3.0
# Los prefijos conservan el orden ordinal al ordenar las clases en los modelos.
PRICE_DIRECTION_CLASSES = (
    "0_baja", "1_neutro", "2_sube",
)
PRICE_DIRECTION_DISPLAY = ("Baja", "Neutro", "Sube")
...
def price_direction_labels(price, future_price):
    ...
    if NEUTRAL_CHANGE_PCT < 0:
        raise ValueError("El umbral neutral debe ser mayor o igual a cero.")
    ...
    labels = np.select(
        [change_pct < -NEUTRAL_CHANGE_PCT,
         change_pct > NEUTRAL_CHANGE_PCT],
        [PRICE_DIRECTION_CLASSES[0], PRICE_DIRECTION_CLASSES[2]],
        default=PRICE_DIRECTION_CLASSES[1],
    )
