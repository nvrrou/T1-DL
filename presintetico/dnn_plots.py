...
from dnn_config import RESULTS_DIR
...
def plot_convergence(history):
    ...
    fig.suptitle("DNN - Curvas de convergencia", fontsize=14, fontweight="bold")
    fig.tight_layout()
    path = os.path.join(RESULTS_DIR, "08_dnn_convergencia.png")
...
def plot_confusion(y_true, y_pred, classes):
    ...
    ax.set_title(f"DNN - Matriz de confusion (Test, Acc={acc:.4f})", fontweight="bold")
    fig.tight_layout()
    path = os.path.join(RESULTS_DIR, "09_dnn_confusion.png")
    fig.savefig(path, dpi=150, bbox_inches="tight", facecolor="white"); plt.close(fig)
    print(f"    Guardado: {path}")
